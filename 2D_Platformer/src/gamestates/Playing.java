...
public class Playing extends State implements Statemethods {
	...
	private MathProblemOverlay mathProblemOverlay;
	private Rain rain;
	...
	private void initClasses() {
		...
		mathProblemOverlay = new MathProblemOverlay(this);
	...
	public void update() {
		...
		// Check if the math problem overlay is visible and needs updating
	    if (mathProblemOverlay.isVisible()) {
	        mathProblemOverlay.update();
	        return;  // Block other updates while the math problem is visible
	    }
		if (paused)
			pauseOverlay.update();
	...
	public void draw(Graphics g) {
		...
		if (paused) {
		 ...
		 // Draw the math problem overlay last to ensure it covers other elements when active
	    if (mathProblemOverlay.isVisible()) {
	        mathProblemOverlay.draw(g);
	    }
	...
	public void keyPressed(KeyEvent e) {
		  ...
		  if (mathProblemOverlay.isVisible()) {
		        mathProblemOverlay.keyPressed(e.getKeyCode());
		        return;  // Prevent other game actions while solving math problems
		    }
	...
	public void setLevelCompleted(boolean levelCompleted) {
		  if (levelCompleted && mathProblemOverlay.isSolved()) {
		        ...
		        if (levelManager.getLevelIndex() + 1 >= levelManager.getAmountOfLevels()) {
		            ...
		            lvlCompleted = true; // This should only be set here to ensure proper transition.
		            loadNextLevel();
		        }
		    }else
		    {
		    	if (!mathProblemOverlay.isVisible()) {  // Only show overlay if not already visible
        	        mathProblemOverlay.setVisible(true); // Show overlay
        	    }
		    }
	}
